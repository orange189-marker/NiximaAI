              ...
              <div className="p-4 rounded-xl bg-zinc-900/90 border border-zinc-700/80 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-white text-xs font-mono focus:outline-none focus:border-zinc-500 placeholder-zinc-600"
                    placeholder="sk-or-v1-..."
                  />
                  {settings.openRouterApiKey && (
                    <button
                      type="button"
                      onClick={() => onUpdateSettings({ openRouterApiKey: '' })}
                      className="px-2.5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white text-xs font-mono transition-colors"
                      title={language === 'uk' ? 'Очистити ключ' : 'Clear key'}
                    >
                ...
                <div className="flex items-center justify-between text-[11px] pt-1">
                  <a
                    ...
                    className="text-orange-400 hover:text-orange-300 transition-colors flex items-center gap-1 font-mono"
                  >
                    <span>{language === 'uk' ? 'Отримати безкоштовний ключ на OpenRouter' : 'Get a free key at openrouter.ai/keys'}</span>
                    <ExternalLink className="w-3 h-3" />
                  </a>
                  <span className="text-zinc-500 font-mono">
                    {language === 'uk' ? '100% безкоштовно • Без карток' : '100% Free • No card'}
                  </span>
                </div>
              </div>
