...
const DISMISS_KEY = 'nixima_release_0_4_card_dismissed';
...
export const ReleaseAnnouncementCard: React.FC<ReleaseAnnouncementCardProps> = ({
  ...
  // Showcase lineup: Nixima-0.4 first, plus top 0.3 models
  const showcaseModels = [
    NIXIMA_MODELS.find(m => m.id === 'nixima-0.4'),
    NIXIMA_MODELS.find(m => m.id === 'nixima-0.3-pro'),
    NIXIMA_MODELS.find(m => m.id === 'nixima-0.3-coder'),
    NIXIMA_MODELS.find(m => m.id === 'nixima-0.3'),
  ...
  return (
    <div className="relative w-full mb-8 rounded-2xl overflow-hidden border border-zinc-700/80 bg-gradient-to-b from-[#14141a]/95 via-[#0e0e13]/95 to-[#09090c]/98 p-5 sm:p-6 shadow-[0_8px_32px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-xl animate-fade-in text-left">
      ...
      <div className="absolute top-0 right-1/4 w-96 h-32 bg-amber-500/[0.04] blur-3xl pointer-events-none rounded-full" />
      <div className="absolute bottom-0 left-10 w-64 h-24 bg-emerald-500/[0.03] blur-2xl pointer-events-none rounded-full" />

      {/* Top row: Badge + Dismiss button */}
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-800/90 border border-amber-500/30 shadow-inner-light">
          <span className="flex h-2 w-2 relative">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-40" />
      ...
      <div className="max-w-2xl mb-5">
        <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-white mb-1.5 font-sans flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded-full text-[11px] font-mono font-semibold bg-gradient-to-r from-amber-400 to-emerald-400 text-black hidden sm:inline-block shadow-[0_0_10px_rgba(245,158,11,0.3)]">
            v0.4 Omni
          </span>
        </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 mb-5">
        {showcaseModels.map((model) => {
          ...
          const is04 = model.id === 'nixima-0.4';
          ...
          return (
            <button
              ...
              className={`p-3 rounded-xl border text-left transition-all duration-200 cursor-pointer flex flex-col justify-between group ${
                isSelected
                  ? is04
                    ? 'bg-amber-950/25 border-amber-500/70 shadow-[0_0_20px_rgba(245,158,11,0.15)] ring-1 ring-amber-500/40'
                  ...
                  : is04
                    ? 'bg-zinc-900/80 hover:bg-zinc-800/80 border-amber-500/30 hover:border-amber-500/60 shadow-sm'
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${
                    isSelected 
                      ? is04 ? 'bg-amber-400 text-black' : 'bg-white text-black'
                      : is04 ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-zinc-800 text-zinc-400'
                  }`}>
      ...
      <div className="pt-4 border-t border-zinc-800/80 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => {
              const flagship = NIXIMA_MODELS.find(m => m.id === 'nixima-0.4') || NIXIMA_MODELS[0];
              if (flagship) onSelectModel(flagship);
            }}
            className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-amber-500/20 to-emerald-500/20 hover:from-amber-500/30 hover:to-emerald-500/30 border border-amber-500/40 text-amber-300 hover:text-white text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5"
          >
            <Zap className="w-3.5 h-3.5 fill-current text-amber-400" />
            <span>{t.releaseAnnouncement.exploreBtn}</span>
          </button>
        </div>

        <div className="text-[11px] font-mono text-zinc-400 flex items-center gap-2">
          <span className="text-amber-300 font-bold">4,000,000 Tokens</span>
          <span>•</span>
          <span>DeepThinking V3.0</span>
          <span>•</span>
          <span>7-Cluster Swarm</span>
          <span>•</span>
          <span className="text-emerald-400 font-bold">2048 Elo</span>
        </div>
      </div>
    </div>
  );
