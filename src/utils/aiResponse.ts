...
- Tool 1 (Search V3.1): Real-time web ingestion and neural source retrieval for: "${prompt.slice(0, 50)}..."
...
3. Огляд інтеграції інструменту Search V3.1 для живого огляду веб-ресурсів (мін. 20 сайтів) та новинних стрічок у реальному часі.`,
        response: `### Часова архітектура та актуальність знань Nixima AI

**Nixima AI не має застарілого зрізу знань 2023 чи 2024 року.** Модель функціонує на базі суверенної архітектури покоління **${modelGen}**, синхронізована з реальним часом (**${dateStr}**) та має прямий доступ до світової мережі через систему **Search V3.1** (мінімум 20 сайтів у базовому режимі, 50–80 у Search V3.1 Mega).

| Параметр | Стан системи |
| :--- | :--- |
| **Поточна дата** | **${dateStr}** (${now.getFullYear()} рік) |
| **Базова архітектура** | \`${model.name}\` (Покоління ${modelGen}) |
| **Живий веб-пошук** | **Search V3.1** (Standard 20+, Fast 20, Mega Swarm 50–80) |
| **Оперативні стрічки новин** | Світові та українські новинні агентства у реальному часі |

#### Як працює пошук актуальної інформації:
1. **Інструмент Search V3.1**: Коли ви запитуєте про сьогоднішні події чи свіжі новини, Nixima в реальному часі сканує мінімум 20 вебсайтів глобальних новинних стрічок (Google News Wire, Reuters, BBC, AP, Укрінформ, Суспільне), а в режимі Mega — від 50 до 80 сайтів.
...
3. Outlining Search V3.1 live web mesh integration (min 20 websites, 50-80 Mega) for real-time news wires and verifiable citations.`,
      response: `### Temporal Architecture & Knowledge Grounding in Nixima AI

**Nixima AI does not operate on a legacy 2023 or 2024 knowledge cutoff.** Operating on the sovereign **${model.name}** (${modelGen} Generation), Nixima is anchored in the present (**${dateStr}**) and equipped with **Search V3.1 Real-Time Web Grounding** (minimum 20 websites scanned, and 50–80 websites in Search V3.1 Mega).
...
| **Live Web Browsing** | **Search V3.1** (Standard 20+, Fast 20, Mega Swarm 50–80) |
| **News Wire Retrieval** | Real-time international news wire feeds |

#### How Real-Time Web Grounding Works:
1. **Search V3.1 Web Mesh**: When asking about breaking developments or today's events, Nixima queries live news wires and indexes in real time across a minimum of 20 websites (and 50–80 in Search V3.1 Mega).
...
> 💡 **Search V3.1:** Для дослідження конкретної новини, законопроєкту чи заяви, активуйте інструмент **Search V3.1** (мін. 20 вебсайтів).`
        };
      }
...
> 💡 **Tip:** To inspect any specific US news event or bill in real time, submit your query with **Search V3.1** enabled for live multi-website verification (20+ sites).`
      };
    }
...
> 💡 **Search V3.1:** Для детального дослідження конкретної теми, задайте уточнююче запитання з активним інструментом **Search V3.1** (мін. 20 вебсайтів).`
      };
    }
...
> 💡 **Tip:** To explore any specific breaking event, country, or technology in greater depth, submit a query with **Search V3.1** enabled for exhaustive, live website inspection (20+ sites).`
    };
  }
...
2. Опис суверенної все-в-одному архітектури: поєднання міркувань (DeepThinking), системного кодування та пошуку (Search V3.1).
...
1. **Повна автономність (Zero Manual Toggles)**:
   - Вам більше не потрібно вручну перемикати режими **DeepThinking V3.0** або **Search V3.1**.
...
| **Автономні інструменти** | Dynamic DeepThinking & Live Search V3.1 Grounding |
...
| **Autonomous Capabilities** | Self-directing DeepThinking & Live Web Search V3.1 |
